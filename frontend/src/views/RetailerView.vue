<template>
  <section class="space-y-6">
    <!-- Nếu không có quyền Retailer -->
    <div
      v-if="!roles.RETAILER"
      class="rounded-xl border border-amber-200 bg-amber-50 p-4 text-xs text-amber-800"
    >
      Bạn chưa có quyền truy cập vai trò <strong>Retailer</strong>. Hãy kết nối
      ví có role phù hợp hoặc nhờ Admin cấp quyền.
    </div>

    <!-- Nội dung chính cho Retailer -->
    <div v-else class="space-y-6">
      <div class="space-y-2">
        <h2 class="text-2xl font-semibold tracking-tight text-slate-900">
          Bảng điều khiển Nhà bán lẻ (Retailer)
        </h2>
        <p class="text-sm text-slate-600 max-w-2xl">
          Nhận lô hàng từ logistics, quản lý quá trình bán hàng, đánh dấu trạng
          thái tiêu thụ và lưu trữ lô đã hoàn thành.
        </p>
      </div>
